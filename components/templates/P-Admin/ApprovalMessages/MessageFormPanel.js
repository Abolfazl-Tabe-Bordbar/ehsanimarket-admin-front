...
import createApprovalMessage from "@/funcs/createApprovalMessage";
import editApprovalMessage from "@/funcs/editApprovalMessage";
...
function MessageFormPanel({
  ...
  const {
    ...
    defaultValues: { user_message: defaultUserMessage },
  });
  ...
  const onSubmit = (data) => {
    ...
    const request =
      mode === "edit"
        ? editApprovalMessage(payload, messageId)
      ...
      <div className="admin-card px-4 py-4 mb-4">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <label
              htmlFor={`approval-user-${mode}-${messageId || "new"}`}
              className="block text-sm font-bold"
            >
              پیام برای کاربر
            </label>
            <textarea
              id={`approval-user-${mode}-${messageId || "new"}`}
              rows={3}
...
export default MessageFormPanel;
