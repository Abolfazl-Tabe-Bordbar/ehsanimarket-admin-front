...
import {
  ...
  getProductComments,
  replyProductComment,
} from "@/funcs/productComments";

function ProductCommentsModal({ product, onClose, onCommentsChange }) {
  ...
  const handleApprove = async (commentId) => {
    const res = await approveProductComment(commentId);
  ...
  const handleReply = async (commentId) => {
    ...
    const res = await replyProductComment(commentId, body);
    Swal.fire({ icon: res.status ? "success" : "error", title: res.message });
    if (res.status) {
    ...
    <div className="admin-modal-overlay items-center">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div>
            <h3 className="font-bold text-base">نظرات محصول</h3>
              ...
              <div key={comment.id} className="border rounded-xl p-4 bg-gray-50">
                ...
                {comment.replies?.map((reply) => (
                  <div key={reply.id} className="mt-3 mr-4 border-r-2 border-[#253c8a]/30 pr-3">
                    <p className="text-xs font-medium text-[#253c8a]">پاسخ {reply.authorName}</p>
